...
//
//		const auto possibleMovesPerIteration = map->GetRobots().size() * 4;
//
//		Moves initialState{};
//
//		std::vector<Moves> CurrentStates = { initialState };
//		CurrentStates.reserve(std::pow(possibleMovesPerIteration, m_solverSafety));
//		auto endCurrentStates = CurrentStates.end();
//
//		std::vector<Moves> tempStates;
//		tempStates.reserve(std::pow(possibleMovesPerIteration, m_solverSafety));
//
